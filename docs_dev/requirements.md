...
FR-002: 画像入力 `/image_raw/compressed` を受け、暗さ加工＋スコア焼き込み（またはログへの確実な記録）を行い、静止画像として `/capture/image_raw` に再パブリッシュできること。  
...
### Variant A: マスト要素の一時無効化

以下は一次対応として許可されたバリエーションであり、恒久化はしない。
- `capture_and_score` はマスト状態に依存しない（FR-002の但し書きを無効化）
- 排他制御（FR-006）は無効化し、`Need to close mast` を返さない
